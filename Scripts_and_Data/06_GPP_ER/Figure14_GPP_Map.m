...
% CSSPv3, ensemble median, and FLUXSat observation
fig_map(:,:,1) = CSSP_MEAN.GPP_mon(:,:,1);
fig_map(:,:,2) = median(MsTMIP_MEAN.GPP_mon,4,'omitnan');
fig_map(:,:,3) = FLUXSat_MEAN.GPP_mon;

% latitudinal means 
OBS_LAT(:,1) = FLUXSat_LAT.GPP_mon;
CSSPv3_LAT(:,1) = CSSP_LAT.GPP_mon(:,1);
CSSPv2_LAT(:,1) = CSSP_LAT.GPP_mon(:,2);
MsTMIP_mid_LAT(:,1) = prctile(MsTMIP_LAT.GPP_mon,50,2);
MsTMIP_upper_LAT(:,1) = prctile(MsTMIP_LAT.GPP_mon,75,2);
MsTMIP_lower_LAT(:,1) = prctile(MsTMIP_LAT.GPP_mon,25,2);

figure(1)
set(gcf,'Units','centimeters','Position',[5,5,32.5,30.5],'color','white');
ha = tight_subplot(3,1,[.055 .055],[.035 .035],[.055 .325]);
ma = tight_subplot(1,1,[.0 .05],[.065 .045],[.725 .045]);
title_name = {'(a) CSSPv3-Simulated GPP','(b) Ensemble Median', '(c) FLUXSat GPP','(d) Mean Latitudinal Profile of GPP'};
...
CSSPv2_valid = CSSPv2_LAT(valid_idx,1);
OBS_valid = OBS_LAT(valid_idx);
...
fig_lat(3) = plot(OBS_valid, LAT_valid, 'LineWidth', 1.5, 'Color', [000,000,000]/255);
...
m_title = title(cell2mat(title_name(4)),'FontSize',14,'FontName','Time New Roman',...
    'HorizontalAlignment','left', 'Units', 'normalized');
set(m_title, 'Position', [0.005, 1.005, 0]);
l = legend([ fig_lat(3),fig_lat(1), fig_lat(2), IQR], ...
    {'FLUXSat','CSSPv3','Ensemble Median', 'Ensemble IQR'}, ...
...
print(figure(1),'./Fig14_GPP_MAP','-dpng','-r300')
